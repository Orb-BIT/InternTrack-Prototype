...
// Session key is defined in auth.js
// DEFAULT_PASSWORD removed — authentication will be handled by the backend via api.js
...
const ROLE_RULES = {
	...
	coordinator: {
		...
		nav: [
			...
			{
				section: "Manage",
				href: "logbook-review.html",
				icon: "fa-book-open",
				text: "Logbook Review",
			},
			{
				section: "Manage",
				href: "doc-approvals.html",
...
// DEMO_USERS removed — user data will come from the backend via api.js/auth.js
...
// Returns prefix to navigate between root and pages/ directory
function getPagePrefix() {
	return isLoginPage() ? "pages/" : "";
}
...
// resolveUser and detectRoleFromId removed — login is now handled via auth.login() which calls the backend API
...
function handleLogin(e) {
	...
	const loginForm = document.getElementById("loginForm");
	const emailInput = document.getElementById("studentNumber");
	const pwInput = document.getElementById("password");
	const err = document.getElementById("loginError");
	const btn = (loginForm || document).querySelector(".btn-login");

	if (!emailInput || !pwInput || !btn) return false;
	...
	setTimeout(async () => {
		...
		if (user && ROLE_RULES[user.role]) {
			window.location.href = getPagePrefix() + ROLE_RULES[user.role].home;
		}
	}, 500);
...
function renderRoleSidebar() {
	...
	html += `<a href="${getIndexPath()}" class="sidebar-link logout-link"><i class="fa fa-sign-out-alt"></i> Logout</a>`;
	nav.innerHTML = html;
}
...
function applyUserProfile() {
	...
	// Compute fallback values for fields that may be null after registration
	const isAvatarUrl = user.avatar && user.avatar.startsWith('http');
	const avatarInitials = auth.generateAvatar(user.name);
	const subtitle = user.subtitle || user.email || "";
	const roleLabel =
		user.roleLabel ||
	...
	document
		.querySelectorAll(
			".topbar-avatar, .settings-avatar, [data-role-avatar]",
		)
		.forEach((el) => {
			if (isAvatarUrl) {
				const img = new Image();
				...
				el.textContent = user.avatar || avatarInitials;
			}
		});
	...
	if (dashboardMeta && user.role === "student") {
		dashboardMeta.innerHTML = `${program || "Student"} &nbsp;|&nbsp; Student No. ${user.id} &nbsp;|&nbsp; ${company}`;
	}

	const profileName = document.querySelector(
		".settings-profile-head h5, .settings-identity h5",
	);
	if (profileName) profileName.textContent = user.name;

	const profileSub = document.querySelector(
		".settings-profile-head p, .settings-identity p",
	);
	if (profileSub) profileSub.textContent = subtitle;

	const rolePill = document.querySelector(".settings-role-pill");
	if (rolePill) rolePill.textContent = roleLabel;

	const settingsMeta = document.querySelectorAll(
		".settings-meta-item strong",
	);
	if (settingsMeta.length >= 1) settingsMeta[0].textContent = company;
	if (settingsMeta.length >= 2)
		settingsMeta[1].textContent =
			user.role === "student"
				? coordinator
				: "Placement Management Office";
	if (settingsMeta.length >= 3) settingsMeta[2].textContent = term;
	...
	const settingsInputs = document.querySelectorAll(
		".account-settings-panel .form-control, .settings-section-card .form-control",
	);
	if (settingsInputs.length >= 4) {
		settingsInputs[0].value = user.name || "";
		settingsInputs[1].value = user.email || "";
		settingsInputs[2].value = program;
		settingsInputs[3].value = contact;
	}
...
function applyCoordinatorSettingsCopy() {
	...
	if (introBlocks[0])
		introBlocks[0].textContent =
			"Manage your coordinator profile, contact details, and workspace defaults used across roster reviews and monitoring.";
	if (introBlocks[1])
		introBlocks[1].textContent =
			"Choose which operational alerts you want to receive for validations, escalations, and site supervision.";
	if (introBlocks[2])
		introBlocks[2].textContent =
			"Update your password and strengthen account protection for coordinator access.";
	...
	if (note) {
		note.textContent =
			"Your coordinator profile powers placement reviews, monitoring follow-ups, and student escalation workflows.";
	}

	const toggles = document.querySelectorAll(".settings-toggle-card");
	if (toggles[0])
		toggles[0].querySelector("strong").textContent =
			"Document validation reminders";
	if (toggles[0])
		toggles[0].querySelector("p").textContent =
			"Receive alerts when placement requirements need coordinator review.";
	if (toggles[1])
		toggles[1].querySelector("strong").textContent =
			"Student escalation alerts";
	if (toggles[1])
		toggles[1].querySelector("p").textContent =
			"Get notified when attendance, journals, or compliance drop below threshold.";
	if (toggles[2])
		toggles[2].querySelector("strong").textContent =
			"Company visit reminders";
	if (toggles[2])
		toggles[2].querySelector("p").textContent =
			"Stay ahead of upcoming supervisor calls, site visits, and follow-ups.";
